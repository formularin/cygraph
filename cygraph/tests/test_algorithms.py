...
def test_get_articulation_points():
    ...
    sample_graphs = {
        'static': {
            'undirected': cg.graph(static=True, directed=False, vertices=list(random_graph)),
            'directed': cg.graph(static=True, directed=True, vertices=list(random_graph))
        },
        'dynamic': {
            'undirected': cg.graph(static=False, directed=False, vertices=list(random_graph)),
            'directed': cg.graph(static=False, directed=True, vertices=list(random_graph))
        }
    }
...
def test_get_shortest_path_dijkstra():
    ...
    for static in [True, False]:
        ...
        undirected_graph = cg.graph(static=static, directed=False,
...
def test_partition_karger():
    ...
    sample_graphs = {
        'static': {
            'undirected': cg.graph(static=True, directed=False, vertices=sample_vertices),
            'directed': cg.graph(static=True, directed=True, vertices=sample_vertices)
        },
        'dynamic': {
            'undirected': cg.graph(static=False, directed=False, vertices=sample_vertices),
            'directed': cg.graph(static=False, directed=True, vertices=sample_vertices)
        }
    }
    for graph in [g for s in sample_graphs for g in sample_graphs[s].values()]:
...
def test_get_strongly_connected_components():
    ...
    for static in [True, False]:
        ...
        test_components_dynamic = alg.get_strongly_connected_components(g,
            static=False)
        for component in test_components:
            assert set(component.vertices) in component_vertices
            test_component_edges = {e[:-1] for e in component.edges}
